...
import { FlashSale, Sale, Trip } from "@/types/tripType"


interface Props {
    flashSale?: FlashSale
    type: "fixed" | "byRoom" | "byRow"
    originPrice: number
    FinalPrice: number[]
    onClick?: () => void
}

const DisplayPrice: FC<Props> = ({ type, originPrice, FinalPrice, flashSale, onClick }) => {

    const hasArrayPrice = Array.isArray(originPrice)

    let isFlashSale = flashSale && flashSale.isActive == true


    if (type == "fixed") {
        return (
            <div className="flex-1 grid grid-rows-3 justify-end">
                <Text className="text-md font-bold text-end"> {formatPrice(FinalPrice)}đ</Text>
                {
                    isFlashSale && <>
                        {flashSale?.sale.type == "fixed" && <div className="flex items-start space-x-1">
                            <Text className="text-slate-800 line-through text-xs">{formatPrice(originPrice)}đ</Text>
                            <Text className="text-white text-xs bg-red-500 rounded-full font-bold px-1">-{flashSale.sale.amount}% </Text>
                        </div>}
                        {flashSale?.sale.type == "percent" && <div className="flex flex-col items-end row-span-2 space-x-1 space-y-1">
                            <Text className="text-slate-800 line-through text-xs ">{formatPrice(originPrice)}đ</Text>
                            <Text className="text-white text-xs bg-green-500 rounded-lg p-1 font-bold">GIẢM {formatPrice(flashSale.sale.percent)}đ </Text>
                        </div>}
